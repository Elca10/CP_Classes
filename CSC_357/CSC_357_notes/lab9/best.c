...
void *lab_malloc(size_t nbytes)
{
   ...
   Header *prev_best_fit = NULL;
   size_t smallest_block_size = (size_t)-1; // Used chatGPT to figure out what to initialize this to
   ...
   for (p = prevp->s.ptr; ; prevp = p, p = p->s.ptr)
   {
      ...
      if (p->s.size >= nunits && p->s.size < smallest_block_size)
      {
         best_fit = p;
         prev_best_fit = prevp;
         smallest_block_size = p->s.size;
      ...
      if (p == freep)
      {
         ...
         if (best_fit) {
            if (best_fit->s.size == nunits) { // block is exact fit
               prev_best_fit->s.ptr = best_fit->s.ptr; // Best fit equivalent of code above
            } else { // there is extra space
               best_fit->s.size -= nunits;
               best_fit = best_fit + best_fit->s.size;
               best_fit->s.size = nunits;
            }
            freep = prev_best_fit;
